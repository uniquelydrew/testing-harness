...
class _JavaAgentPointDriver:
    ...
    def capture_at_point(self, x, y, *, process_id=None):
        ...
        if process_id != 4321:
            raise LookupError("wrong process")
        return _captured("swing", 4321)


def test_point_capture_scopes_mixed_java_agent_to_x11_owner(monkeypatch):
    java_agent = _JavaAgentPointDriver()
    javafx = SimpleNamespace(available=False)
    atspi = SimpleNamespace(available=False)
    service = SimpleNamespace(java_agent_driver=java_agent, javafx_driver=javafx, driver=atspi)
    ...
    assert result.framework == "swing"
    assert java_agent.calls == [(100, 200, 4321)]
