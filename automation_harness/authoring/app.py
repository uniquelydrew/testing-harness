...
class AuthoringApp:
    ...
    def _build_objects(self) -> None:
        ...
        self.object_filter.set_placeholder_text("Object name, description, or type")
        ...
        self.object_store = Gtk.TreeStore(str, str, str, str)
        self.object_tree = Gtk.TreeView(model=self.object_store)
        for index, (title, width) in enumerate((
            ("Object", 280), ("Rev", 50), ("Type", 120), ("Actions", 220),
        )):
            renderer = Gtk.CellRendererText()
            column = Gtk.TreeViewColumn(title, renderer, text=index)
            column.set_resizable(True)
            column.set_min_width(width)
            self.object_tree.append_column(column)
    ...
    def refresh_objects(self) -> None:
        ...
        if hasattr(self, "step_object"):
            for definition in sorted(
                definitions.values(),
                key=lambda item: item.component_id.casefold(),
            ):
                self.step_object.append(
                    definition.component_id,
                    definition.component_id,
                )

        self.object_tree.expand_all()
        total = len(definitions)
        self.object_count.set_text(
            "%d of %d objects" % (len(shown_ids), total)
            if query else "%d objects" % total
        )
        self._select_value(self.object_tree, selected)
        if hasattr(self, "step_object"):
            available = {
                definition.component_id for definition in definitions.values()
            }
            self.step_object.set_active_id(
                selected_for_step
                if selected_for_step in available
                else (next(iter(sorted(available)), None))
            )
    ...
    def show_object(self) -> None:
        ...
        payload = {
            ...
            "actions": sorted(item.value for item in definition.semantic_actions),
            "expected_states": dict(definition.expected_states),
            "subobjects": {
                key: dict(value) for key, value in definition.subobjects.items()
            },
            "locator_status": properties.get("locator_status", "ready"),
            "advanced_locator": {
                "framework": definition.framework,
                "native_class": definition.native_class,
                "strategies": [
                    {"type": item.type, **item.options}
    ...
    def _configure_action(self, definition, component=None):
        if not definition.inputs:
            return {}
        dialog = Gtk.Dialog(
            title="Configure %s" % definition.name,
        ...
        for item in definition.inputs:
            ...
            label = Gtk.Label(
                label=item.name + (" *" if item.required else "")
            )
            label.set_xalign(0)
    ...
    def _build_recorded_objects(self, repository, interactions):
        ...
        for index, interaction in enumerate(updated):
            ...
            if outcome.error is not None:
                # Leave unresolved/provisional interactions reviewable. They
                # cannot be added to a Test until a durable materialization
                # succeeds.
                if outcome.provisional_component_ids:
                    repository = outcome.repository
                    updated[index] = outcome.interaction
                    for component_id in outcome.created_component_ids:
                        if component_id not in changed_ids:
                            changed_ids.append(component_id)
                continue
            repository = outcome.repository
            updated[index] = outcome.interaction
            for component_id in outcome.changed_component_ids:
                if component_id not in changed_ids:
                    changed_ids.append(component_id)
        return repository, updated, changed_ids
    ...
    def _select_value(tree, value, column=0):
        ...
        def visit(iterator):
            while iterator is not None:
                ...
                found = visit(child) if child is not None else None
                if found is not None:
                    return found
                iterator = model.iter_next(iterator)
            return None

        iterator = visit(model.get_iter_first())
...
def _recorded_component_id(target, repository):
    return unique_component_name(repository.components, target)
