"""Predicate evaluation used by semantic completion conditions."""
...
def compare(actual: Any, operator: str, expected: Any) -> bool:
    ...
    try:
        ...
        raise PredicateError(f"cannot evaluate {actual!r} {operator} {expected!r}: {exc}") from exc


def evaluate_state(state: Any, expression: Mapping[str, Any]) -> bool:
    if "all" in expression:
        return all(evaluate_state(state, child) for child in _children(expression, "all"))
    if "any" in expression:
        return any(evaluate_state(state, child) for child in _children(expression, "any"))
