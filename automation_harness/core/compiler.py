...
class CompiledTest:
    ...
    def validate_runtime(self, registry: StepRegistry) -> list[str]:
        ...
        for step_id, dependency in steps.items():
            ...
            if installed.implementation_digest != expected:
                issues.append(
                    f"step {step_id!r} implementation mismatch: compiled={expected}, installed={installed.implementation_digest}"
                )
        return issues

    def component_repository(self) -> ComponentRepository:
        components = self.document["dependencies"].get("components", {})
        return ComponentRepository.from_document({"version": 3, "components": components}, source="compiled artifact")
    ...
    def runtime_plan(self) -> TestPlan:
        ...
        return TestPlan(
            ...
            steps=tuple(calls),
            objects=dict(components) if isinstance(components, Mapping) else {},
            step_definitions={},
        )
    ...
    *,
    ...
    for index, call in enumerate(plan.steps):
        ...
        for output_name, variable_name in sorted(call.outputs.items()):
            producers[variable_name] = {"invocation_id": call.node_id, "output": output_name}
        refs = sorted(_collect_refs(call.inputs) | _collect_refs(call.scope) | _collect_refs(call.completion))
        for path in refs:
            consumers.setdefault(path.split(".", 1)[0], []).append({"invocation_id": call.node_id, "path": path})
        _collect_component_references(call.inputs, components, referenced_components)
        _collect_component_references(call.scope, components, referenced_components)
        _collect_component_references(call.completion, components, referenced_components)
        instructions.append({
            "index": index,
...
def _collect_component_references(value: Any, repository: ComponentRepository, found: set[str]) -> None:
    if isinstance(value, Mapping):
        for key, item in value.items():
            if key in {"component_id", "object", "component", "window", "expected"} and isinstance(item, str):
                if repository.contains(item):
                    found.add(repository.get(item).component_id)
            _collect_component_references(item, repository, found)
