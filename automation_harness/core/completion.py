"""Execution of semantic step completion contracts."""
...
def await_step_completion(
    ...
    contract = _resolve_completion_value(context, dict(call.completion))
    ...
    try:
        wait_for(
            lambda: _evaluate(context, condition),
            timeout=timeout,
            interval=interval,
...
def _resolve_completion_value(context, value: Any) -> Any:
    if isinstance(value, PlanVariableRef):
        if context.globals is None:
            raise CompletionError(f"cannot resolve completion variable {value.path!r} without test globals")
        return context.globals.get(value.path)
    if isinstance(value, Mapping):
        return {str(key): _resolve_completion_value(context, item) for key, item in value.items()}
    if isinstance(value, list):
        return [_resolve_completion_value(context, item) for item in value]
    if isinstance(value, tuple):
        return tuple(_resolve_completion_value(context, item) for item in value)
    return value
...
def _infer_condition(context, call: StepCall, inputs: Mapping[str, Any]) -> Mapping[str, Any] | None:
    ...
    if call.step_id == "gui.text.set" and isinstance(inputs.get("value"), str):
    ...
    if expected:
        return {
            "object": component_id,
            "all": [{"state": name, "equals": value} for name, value in sorted(expected.items())],
        }
    return None
...
def _action_name(step_id: str, inputs: Mapping[str, Any]) -> str | None:
    direct = {
        "gui.text.set": "set_text",
        "gui.selection.select": "select",
        "gui.button.click": "click",
    }
...
def _evaluate(context, condition: Mapping[str, Any]) -> bool:
    if "all" in condition and "object" not in condition:
        return all(_evaluate(context, child) for child in _condition_list(condition, "all"))
    if "any" in condition and "object" not in condition:
        return any(_evaluate(context, child) for child in _condition_list(condition, "any"))
    if "not" in condition and "object" not in condition:
        child = condition["not"]
        if not isinstance(child, Mapping):
            raise CompletionError("completion not must contain a mapping")
        return not _evaluate(context, child)
    ...
    try:
        ...
        return state_name in {"absent", "present", "exists"} and expected in {True, False} and (
            (state_name == "absent" and expected is True)
            or (state_name in {"present", "exists"} and expected is False)
        )
